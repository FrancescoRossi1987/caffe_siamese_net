...
class multilabel(caffe.Layer):
    ...
    def setup(self,bottom,top):
        ...
        top[1].reshape(par.batchsize_dic[self.phase],par.channel,par.height,par.width)
        top[2].reshape(par.batchsize_dic[self.phase],1)
    ...
    def forward(self,bottom,top):
        blob=self.get_batch()#blob=[[data,data_p,sim],[data,data_p,sim],....[data,data_p,sim]]
        num_labels=len(blob[0][1])
        for i in xrange(par.batchsize_dic[self.phase]):
            top[0].data[i,...]=blob[i][0] #data
            top[1].data[i,...]=blob[i][1] #data_p
            top[2].data[i,...]=int(blob[i][2])#sim
